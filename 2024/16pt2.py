...
from grid import CLOCKWISE_DIRS,Grid
...
while queue:
    cur_y, cur_x, cur_dir, cur_score,history = queue.popleft()
    ...
    if grid.get_node_val(cur_y,cur_x) == "E":
        if cur_score not in lowest.keys():
            lowest[cur_score] = []
        lowest[cur_score] += history
    ...
    if is_valid(y_straight,x_straight,grid) and next_score_straight <= lowest_cost.get((y_straight, x_straight, cur_dir), float("inf")):
        ...
        queue.append((y_straight, x_straight, cur_dir, next_score_straight, tuple([x for x in history] + [(cur_y,cur_x)])))
    ...
    for turn_indicator in ((cur_dir +1) % 4,(cur_dir -1) % 4):
        ...
        if is_valid(y_turn,x_turn,grid) and next_score_turn <= lowest_cost.get((y_turn,x_turn, turn_indicator), float("inf")):
            ...
            queue.append((y_turn,x_turn, turn_indicator, next_score_turn, [x for x in history] + [(cur_y,cur_x)]))

print(len(set(lowest[min(lowest.keys())])) + 1)